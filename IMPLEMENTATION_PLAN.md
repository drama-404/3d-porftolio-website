...
3. Ran the current app:
   `npm run dev`

Comment: The rest of the implementation plan was planned before i cloned the git and used the exact code of my inspiration, so there might need to be some adjustments in tech stack or details. Read it carefully and grasp the idea of what needs to be done but don't focus on the exact code snippets because you might need to implement it differently (to adjust to the tech stack that the original creator chose).
